...
class HuobisGateway(BaseGateway):
    ...
    def get_err_alert(self):
        if abs(self.rest_api.err_alert_time - datetime.now()) < timedelta(minutes=3):
            return True
        return False
...
class HuobisRestApi(RestClient):
    ...
    def __init__(self, gateway: BaseGateway):
        """"""
        super().__init__()
        self.err_alert_time = datetime.now() - timedelta(minutes=5)
    ...
    def check_error(self, data: dict, func: str = "") -> bool:
        ...
        if str(error_code) in ['1079', '1078', '1004']:
            self.err_alert_time = datetime.now()
