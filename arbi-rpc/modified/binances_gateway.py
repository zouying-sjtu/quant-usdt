...
DIRECTION_VT2BINANCES: Dict[Direction, str] = {
    (Direction.LONG, Offset.OPEN):   ("BUY", "LONG"),
    (Direction.LONG, Offset.CLOSE):  ("BUY", "SHORT"),
    (Direction.SHORT, Offset.OPEN):  ("SELL", "SHORT"),
    (Direction.SHORT, Offset.CLOSE): ("SELL", "LONG"), 
}

DIRECTION_BINANCES2VT: Dict[Direction, str] = {
    ("BUY", "LONG"):   (Direction.LONG, Offset.OPEN),
    ("BUY", "SHORT"):  (Direction.LONG, Offset.CLOSE),
    ("SELL", "SHORT"): (Direction.SHORT, Offset.OPEN) ,
    ("SELL", "LONG"):  (Direction.SHORT, Offset.CLOSE), 
}
...
class BinancesGateway(BaseGateway):
    ...
    def get_err_alert(self):
        # binance don't have err-checking
        return False
...
class BinancesRestApi(RestClient):
    ...
    def send_order(self, req: OrderRequest) -> str:
        ...
        direction, offset = DIRECTION_VT2BINANCES[(req.direction, req.offset)]
    ...
    def on_query_order(self, data: dict, request: Request) -> None:
        ...
        for d in data:
            ...
            if not order_type:
                continue
            direction, offset = DIRECTION_BINANCES2VT[d["side"], d["positionSide"]]
            order = OrderData(
...
class BinancesTradeWebsocketApi(WebsocketClient):
    ...
    def on_order(self, packet: dict) -> None:
        ...
        if not order_type:
            return
      #  print('ord_data:',ord_data)
        direction, offset = DIRECTION_BINANCES2VT[ord_data["S"], ord_data["ps"]]
